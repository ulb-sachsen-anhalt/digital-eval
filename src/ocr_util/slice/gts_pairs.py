...
class TrainingSets:
    ...
    def __init__(self, path_ocr_data, path_image_data, output_dir):
        ...
        self.xml_data = ET.parse(path_ocr_data).getroot()
    ...
    def write_pair(self, text_line: TextLine,
        ...
        if not output_dir.is_dir():
            output_dir.mkdir()
        gt_txt_name = f"{data_name}_{text_line.element_id}{SUFFIX_GT_TXT_}"
        gt_txt_path = output_dir / gt_txt_name
        img_name = f"{data_name}_{text_line.element_id}{SUFFIX_GT_IMG_TIF}"
...
def extract_rectangular_frame(image_handle, text_line: PageLine):
    ...
    Frame created as new copy in memory, otherwise strange artefacts
    ...
    try:
        ...
        the_msg = f"[{text_line.element_id}] extract_rectangular_frame: {rtw.args}"
        raise ExtractPairException(the_msg) from rtw
